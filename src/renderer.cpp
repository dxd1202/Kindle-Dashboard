...
// 这是一个简化的画字函数
void draw_text(unsigned char* bitmap, int width, int height, const char* text, int x, int y,
               int size) {
    // 读取字体文件
    ...
    for (int i = 0; text[i]; ++i) {
        ...
        int byteOffset = (y + ascent * scale + y0) * width + (pen_x + lsb * scale + x0);
        stbtt_MakeCodepointBitmap(
            &font, bitmap + byteOffset, x1 - x0, y1 - y0, width, scale, scale, text[i]);
